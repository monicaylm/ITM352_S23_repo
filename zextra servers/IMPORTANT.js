...
app.post("/register", function (request, response, next) {
	...
	if (typeof user_data[username] != "undefined") {
		errors[
			`username_taken`
		] = `${username} is already registered! Please login.`;
	}
	...
	if (name == "") {
		errors["no_name"] = `You need to add your name!`;
	}

	// if password field is blank, output error
	if (password == "") {
		errors["no_password"] = `You need to create a password!`;
	...
	if (Object.keys(errors).length === 0) {
		...
		fs.writeFileSync(__filename, JSON.stringify(user_data));
		let params = new URLSearchParams(selected_qty);
		params.append("email", username)
		params.append("name", name);
		response.redirect("./invoice.html?" + params.toString());
